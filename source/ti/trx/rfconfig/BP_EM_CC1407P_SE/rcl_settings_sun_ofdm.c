...
// PHY                                   779 - 930 MHz - IEEE 802.15.4 - SUN OFDM - Wi-SUN, Japan
//     State                             Experimental
// PHY abbreviation                      sunOfdm
//
//
// PHY PROPERTIES
//
...
#include "rcl_settings_sun_ofdm.h"


// Configuration: FRONTEND
const uint32_t LRF_frontendRegConfig_sunOfdm[] =
{
...
const uint32_t LRF_frontendRegConfig_sunOfdm_byteCount = sizeof(LRF_frontendRegConfig_sunOfdm);


// Configuration: MAIN
const uint32_t LRF_mainRegConfig_sunOfdm[] =
{
    0x000080E4,    // 17 segment(s), total length = 228
    ...
    0x002C9003,    //     Data structure 32-bit no-copy region (pointer byte position = 44, count = 4)
    0x00000003,    //         TRX_wisunMdrMappingTable : numEntries
    0x00012730,    //         TRX_wisunMdrMappingTable : entry
    0x00042340,    //         TRX_wisunMdrMappingTable : -
    0x00102150,    //         TRX_wisunMdrMappingTable : -
    0x0000300D,    //     HW sparse region (address/value pairs, count = 14)
    ...
    0x33A01001,    //     HW 16-bit region (start address = 0x33A0, count = 2)
    ...
    0x00014002,    // Segment length = 2 (OFDM_OPTION = 1, 3MCS0, 3MCS16, 4MCS01, 4MCS26)
    0x00003000,    //     HW sparse region (address/value pairs, count = 1)
    0x30B80000,    //         LRFDRFE.RX0BDEC
    0x001F0002,    // Segment length = 2 (OFDM_OPTION = 2, 3MCS0, 3MCS16, 4MCS01, 4MCS26)
    0x33A81001,    //     HW 16-bit region (start address = 0x33A8, count = 2)
    0x00080003,    //         RFE_FW.IFADCPEAKTHR          RFE_FW.AGCCFG5
    0x00184009,    // Segment length = 9 (OFDM_OPTION = 1, 2, 3MCS0, 3MCS16)
    ...
    0x001E0002,    // Segment length = 2 (OFDM_OPTION = 3MCS0, 3MCS16, 4MCS01, 4MCS26)
    0x00003000,    //     HW sparse region (address/value pairs, count = 1)
    0x339050B0,    //         LRFDRFE.SPARE0
    0x00074002,    // Segment length = 2 (OFDM_OPTION = 1, 4MCS01, 4MCS26)
    0x00003000,    //     HW sparse region (address/value pairs, count = 1)
    0x3118A8C6,    //         RFE_FW.AGCCFG2
    0x00070003,    // Segment length = 3 (OFDM_OPTION = 2, 3MCS0, 3MCS16)
    0x00003001,    //     HW sparse region (address/value pairs, count = 2)
    0x311C206A,    //         RFE_FW.AGCCFG3
    0x611043AF,    //         LRFDOCE.SUNCORR4
    0x00190003,    // Segment length = 3 (OFDM_OPTION = 2, 4MCS01, 4MCS26)
    ...
    0x001E4004,    // Segment length = 4 (OFDM_OPTION = 1, 2)
    ...
    0x00060014,    // Segment length = 20 (OFDM_OPTION = 3MCS0, 3MCS16)
    0x0000A001,    //     Data structure 32-bit region (start byte position = 0, count = 2)
    ...
    0x00180016,    // Segment length = 22 (OFDM_OPTION = 4MCS01, 4MCS26)
    ...
    0x30C81002,    //     HW 16-bit region (start address = 0x30C8, count = 3)
    0x163F1307,    //         LRFDRFE.RX0MAGNCTRL1         LRFDRFE.RX0MAGNCTRL0
    0x0000163F,    //         -                            LRFDRFE.RX0MAGNCTRL2
    0x31BCB001,    //     HW masked region (mask/value pairs, count = 2)
    ...
    0x001F4016,    // Segment length = 22 (OFDM_OPTION = 1)
    0x0000A001,    //     Data structure 32-bit region (start byte position = 0, count = 2)
    ...
    0x0000242D,    //         -                            RFE_FW.PHYAGCCFG4
    0x33A81001,    //     HW 16-bit region (start address = 0x33A8, count = 2)
    0x000A0002,    //         RFE_FW.IFADCPEAKTHR          RFE_FW.AGCCFG5
    0x60D01001,    //     HW 16-bit region (start address = 0x60D0, count = 2)
    0xC400D05F,    //         LRFDOCE.SUNDEM0              LRFDOCE.SUNMOD0
    0x00010011,    // Segment length = 17 (OFDM_OPTION = 2)
    0x00003007,    //     HW sparse region (address/value pairs, count = 8)
    ...
    0x00020002,    // Segment length = 2 (OFDM_OPTION = 3MCS0)
    0x00003000,    //     HW sparse region (address/value pairs, count = 1)
    0x617840CC,    //         OCE_MAIN_FW.FWCFG2
    0x00040002,    // Segment length = 2 (OFDM_OPTION = 3MCS16)
    0x00003000,    //     HW sparse region (address/value pairs, count = 1)
    0x6178406D,    //         OCE_MAIN_FW.FWCFG2
    0x00080005,    // Segment length = 5 (OFDM_OPTION = 4MCS01)
    ...
    0x00100005,    // Segment length = 5 (OFDM_OPTION = 4MCS26)
    0x00003003,    //     HW sparse region (address/value pairs, count = 4)
...
const uint32_t LRF_mainRegConfig_sunOfdm_byteCount = sizeof(LRF_mainRegConfig_sunOfdm);

// TRX_TxPowerLevels data structure
const TRX_TxPowerLevels TRX_txPowerLevels_sunOfdm = {
    .numEntries  = 0x18,
    .powerLevels = {
        { .fraction = 0, .dBm = -30 },
        { .fraction = 0, .dBm = -20 },
...
// TRX_TxPowerLevels data structure
const TRX_TxPowerLevels TRX_txPowerLevelsHigh_sunOfdm = {
    .numEntries  = 0x13,
    .powerLevels = {
        { .fraction = 0, .dBm =   0 },
        { .fraction = 0, .dBm =   5 },
