...
// PHY                                779 - 930 MHz - IEEE 802.15.4 - SUN FSK - Wi-SUN, Japan
//     State                          Experimental
// PHY abbreviation                   sunFsk
//
//
// PHY PROPERTIES
//
...
#include "rcl_settings_sun_fsk.h"


// Configuration: FRONTEND
const uint32_t LRF_frontendRegConfig_sunFsk[] =
{
...
const uint32_t LRF_frontendRegConfig_sunFsk_byteCount = sizeof(LRF_frontendRegConfig_sunFsk);


// Configuration: MAIN
const uint32_t LRF_mainRegConfig_sunFsk[] =
{
    0x00008116,    // 35 segment(s), total length = 278
    0x00000073,    // Segment length = 115
    ...
    0x002C9005,    //     Data structure 32-bit no-copy region (pointer byte position = 44, count = 6)
    0x00000005,    //         TRX_wisunMdrMappingTable : numEntries
    ...
    0x3370100A,    //     HW 16-bit region (start address = 0x3370, count = 11)
    ...
    0x00804002,    // Segment length = 2 (FSK_MODE = 1A, 1B, 2A, 2B, 3, 4A, 4B)
    0x00003000,    //     HW sparse region (address/value pairs, count = 1)
    0x33A80001,    //         RFE_FW.AGCCFG5
    0x00024002,    // Segment length = 2 (FSK_MODE = 1A, 2A, 2B, 3, 4A, 4B, 5)
    0x00003000,    //     HW sparse region (address/value pairs, count = 1)
    ...
    0x00E04003,    // Segment length = 3 (FSK_MODE = 1A, 1B, 2A, 2B, 3)
    0x00003001,    //     HW sparse region (address/value pairs, count = 2)
    0x10881F2F,    //         PBE_FW.MCECMDPAR0
    0x23A00002,    //         LRFDMCE.FWPRECTRL
    0x004A4003,    // Segment length = 3 (FSK_MODE = 1A, 2A, 3, 4A, 5)
    ...
    0x00FC4006,    // Segment length = 6 (FSK_MODE = 1A, 1B)
    ...
    0x00FA4004,    // Segment length = 4 (FSK_MODE = 1A, 2A)
    0x00003002,    //     HW sparse region (address/value pairs, count = 3)
    ...
    0x00FE4007,    // Segment length = 7 (FSK_MODE = 1A)
    0x0000A001,    //     Data structure 32-bit region (start byte position = 0, count = 2)
    ...
    0x00080003,    // Segment length = 3 (FSK_MODE = 2B)
    0x31BCB001,    //     HW masked region (mask/value pairs, count = 2)
    ...
    0x00800009,    // Segment length = 9 (FSK_MODE = 5)
    0x0010A000,    //     Data structure 32-bit region (start byte position = 16, count = 1)
...
const uint32_t LRF_mainRegConfig_sunFsk_byteCount = sizeof(LRF_mainRegConfig_sunFsk);

// TRX_TxPowerLevels data structure
const TRX_TxPowerLevels TRX_txPowerLevels_sunFsk = {
    .numEntries  = 0x18,
    .powerLevels = {
        { .fraction = 0, .dBm = -30 },
        { .fraction = 0, .dBm = -20 },
...
// TRX_TxPowerLevels data structure
const TRX_TxPowerLevels TRX_txPowerLevelsHigh_sunFsk = {
    .numEntries  = 0x13,
    .powerLevels = {
        { .fraction = 0, .dBm =   0 },
        { .fraction = 0, .dBm =   5 },
