...
TRX Host RF Driver. This example is meant to be used with the Packet TX example
or SmartRF Studio. For every packet received, the red LED is toggled.

Extended Radio Commands
-----------------------
The GenFSK variant of this example (rfPacketRxGenFSK.c) can receive using
either the regular receive command or the extended receive command
(TRX_RadioCommand_ExtReceive). Set `USE_EXTENDED_COMMANDS` to 1 at the top of
rfPacketRxGenFSK.c to select the extended command. 
When using the extended commands:
...
Example Usage
-------------
Run the example. On another board, run the RF Packet TX example.
...
2. Explicitly configures an RX command including
    * Frequency
    * PHY and Modem
    * Trigger Type
    * Payload Buffer
    * Repeat
3. Sends the repeating RX command to the TRX using the TRX_Host_storeCmds API which then starts receiving
4. Receives a callback from the TRX RF Host driver when a packet is received and toggles CONFIG_PIN_RLED
