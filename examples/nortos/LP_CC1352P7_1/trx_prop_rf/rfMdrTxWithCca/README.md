...
The WiSUN Modulation and Data Rate (MDR) negotiation transmit with Clear
Channel Assessment (CCA) example shows a simplified implementation of
transmitting MDR packets with a Carrier Sense (CS) command prior to each TX.
The PHYs, channels, and payload lengths are user selectable and the MDR
transmission is implemented as a chain of CS --> TX (Header) --> CS --> TX (Payload) .When CONFIG_GPIO_BTN1 is pressed, the example will send the user selected number
